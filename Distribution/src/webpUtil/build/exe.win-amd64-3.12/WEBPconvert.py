...
class WEBPconvert:
    ...
    def fileConvert(self, savePath):
        ...
        try:
            print(f"Input file: {self.inputPath}")  # Debug log
            output_path = os.path.abspath(savePath)
            if self.isAnimated():
                # Convert animated WEBP to GIF
                gifPath = f"{os.path.splitext(output_path)[0]}.gif"
                self.convertToGif(gifPath)
                print(f"Animated WebP converted to: {gifPath}")  # Debug log
                if os.path.exists(gifPath):
                    return gifPath
                else:
                    raise ValueError(f"GIF not created: {gifPath}")
            else:
                # Convert static WEBP to PNG
                pngPath = f"{os.path.splitext(output_path)[0]}.png"

                # Path normalization
                from pathlib import Path
                self.inputPath = Path(self.inputPath).as_posix()
                pngPath = Path(pngPath).as_posix()

                # Determine the path to dwebp.exe in the build directory
                dwebp_executable_path = os.path.join(
                    os.path.dirname(sys.executable), "dwebp.exe"
                )

                # Add the directory of dwebp.exe to the PATH environment
                os.environ["PATH"] += os.pathsep + os.path.dirname(dwebp_executable_path)

                # Log paths for debugging
                print(f"dwebp.exe path: {dwebp_executable_path}")
                print(f"Input file: {self.inputPath}")
                print(f"Output file: {pngPath}")

                    # Call dwebp.exe directly using subprocess
                command = [dwebp_executable_path, self.inputPath, "-o", pngPath]
                print(f"Executing command: {' '.join(command)}")  # Debug log
                result = subprocess.run(command, capture_output=True, text=True)

                # Debug output
                print(f"dwebp stdout: {result.stdout}")
                print(f"dwebp stderr: {result.stderr}")

                if result.returncode != 0:
                    raise ValueError(f"dwebp failed with error: {result.stderr}")
                
                print(f"Static WebP converted to: {pngPath}")  # Debug log
                if os.path.exists(pngPath):
                    return pngPath
                else:
                    raise ValueError(f"PNG not created: {pngPath}")
        except Exception as ex:
            print(f"Error during file conversion: {ex}")  # Debug log
